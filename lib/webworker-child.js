...
// new Worker() object.
//
//      usage: node webworker-child.js [-S] <sock> <script>
//
...
//      Options:
//
//          -S               Create a context appropriate for a shared
//                           worker rather than a dedicated worker
...
//
// XXX: Set .name property as specified by the SharedWorker constructor. Get
//      this via the -S option.
var createContext = function(loc, shared, port) {
    var ctx = {};
...
if (argv.length != 2) {
    throw new Error('usage: node webworker-child.js [-S] <sock> <script>');
}

var pp = new ParentProcess(argv[0], argv[1], isShared);
...
var ctx = createContext(loc, isShared, port);
