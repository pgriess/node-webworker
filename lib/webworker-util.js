...
var MsgStream = function(s) {
    ...
    if (msgpack) {
        (function() {
            // Our msgpack.Stream instance which we're interfacing with, as
            // it will manage framing for us
            var mpStream = new msgpack.Stream(s);

            // Send the given JavaSript object. If 'fd' is provided, it is
            // sent as a file descriptor along with the message.
            self.send = function(v, fd) {
                mpStream.send(getMsgObj(v, fd), undefined, fd);
            };

            mpStream.addListener('msg', msgHandler);
        })();
    } else if (ws) {
        (function() {
            self.send = function(v, fd) {
                s.send(JSON.stringify(getMsgObj(v, fd)), fd)
            };

            s.addListener('data', msgHandler);
        })();
    } else {
        throw new Error(
            'Neither node-msgpack nor node-websocket-client could be found!'
        );
    }
