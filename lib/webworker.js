...
var WorkerMaster = function(path) {
    ...
    //
    // Note that this is a synchronous implementation. We don't want workers
    // being started up until we're done initializing.
    self.start = function() {
        try {
            fs.unlinkSync(path);
        } catch (e) {
            if (e.errno != netBinding.ENOENT) {
                throw e;
            }
        }

        var fd = netBinding.socket('unix');
