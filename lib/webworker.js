...
// of ports can be associated with a given Worker Process, but a given port
// can only be assigned to a single process. This is done at construction
// time.
var Port = function(wp) {
    var self = this;

    wp.addListener('err', function(e) {
    ...
    wp.addListener('message', function(msg, fd) {
        process.nextTick(function() {
            var e = { data : msg };
            if (fd) {
                e.fd = fd;
    ...
    wp.addListener('exit', function(code, signal) {
        process.nextTick(function() {
            if (self.onexit) {
...
//
// Implements the external API for shared workers
var SharedWorker = function(src, name, opts) {
    var self = this;

    // XXX: Need to verify that (src, name) tuples in cache hits
    //      are identical to values provided to the constructor
    ...
    // Export our external API
    self.port = port;
    self.__defineSetter__('port', function(v) {
        throw new Error('The \'port\' property is read only.');
    });
};
