...
//
// This is the internal implementation of much Web Worker functionality. It
// exports an interface that is optimized for NodeJS, not attempting to 
// conform to the W3C standards. An instance of this class is meant to be
// used to be wrapped when providing an extnernal API.
//
// Each worker communicates with the master over a UNIX domain socket rooted
// in SOCK_DIR_PATH.
var WebWorkerImpl = function(src, opts) {
...
sys.inherits(WebWorkerImpl, events.EventEmitter);

var WebWorker = function(src, opts) {
    events.EventEmitter.call(this);

    var self = this;

    // This instance will do all of the real work for us
    var impl = new WebWorkerImpl(src, opts);
   
    // Wire up NodeJS events to a W3C-stlye on* API
    impl.addListener('err', function(e) {
    ...
    impl.addListener('message', function(msg, fd) {
        process.nextTick(function() {
            var e = { data : msg };
            if (fd) {
                e.fd = fd;
...
sys.inherits(WebWorker, events.EventEmitter);
