...
var WorkerImpl = function(w, src) {
    ...
    // talk to the worker. It contains [type, data] tuples.
    var msgQueue = [];
    ...
    self.start = function() {
        cp = child_process.spawn(
            process.argv[0],
            [
                path.join(__dirname, '..', '..', 'libexec', 'node', 'worker.js'),
                SOCK_PATH,
                'file://' + src
            ],
            undefined,
    ...
    self.postMessage = function(msgType, msg) {
...
var Worker = function(src) {
    ...
    self.postMessage = function(msg) {
        impl.postMessage(wwutil.MSGTYPE_USER, msg);
    };
