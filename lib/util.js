...
// A port object
// 
// This is our implementation of the Web Worker 'ports' concept. Any number
// of ports can be associated with a given source (e.g. WorkerProcess
// instance), but a given port can only be assigned to a single source.
// This is done at construction time.
//
// This object is provided as-is to client code -- it must implement a safe
// and complaint API.
var Port = function(src, post) {
    var self = this;

    src.addListener('err', function(e) {
    ...
    src.addListener('message', function(msg, fd) {
        process.nextTick(function() {
            var e = { data : msg };
            if (fd) {
                e.fd = fd;
    ...
    src.addListener('exit', function(code, signal) {
        process.nextTick(function() {
            if (self.onexit) {
    ...
    self.addEventListener = function(type, func) {
        self.addListener(type, func);

        // XXX: The W3C API requires that we export a 'start' method which
        //      must be called to begin the event pump. Also, this should
        //      only be visibiele and available to shared workers (I think).
    };
};
sys.inherits(Port, events.EventEmitter);
exports.Port = Port;
