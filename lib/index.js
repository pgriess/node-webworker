...
var WorkerProcess = function(src, opts) {
    ...
    var terminateTimeoutID = undefined;
  
    // Whether or not we're in the midst of shutting down; used for
    ...
    wsSrv.addListener('connection', function(s) {
        ...
        // Now that we've gotten our (one) connection, close the server
        // itself. The connection remains open -- this just closes the
        // listening socket.
        wsSrv.close();
    });
    ...
    self.start = function() {
        wsSrv.addListener('listening', function() {
            ...
            cp.addListener('exit', function(code, signal) {
                ...
                // Only close the serve if it's still running
                //
                // Although we shut down the server on first connection, we
                // can still get here if the child process exited before
                // successfully creating a connection.
                if (wsSrv.fd) {
                    wsSrv.close();
                }
    ...
    self.terminate = function(timeout) {
        assert.ok(!cp || cp.pid == pid || (!cp.pid && !pid));

        timeout = (timeout === undefined) ?  1000 : timeout;

        // We've already initiated termination; nothing to do
        if (isTerminating) {
            return;
        }
        ...
        // We don't have a child process (either because we haven't created
        // one yet, or its already shut down); nothing else to do
        if (!cp || !cp.pid) {
            return;
        }
        ...
        if (timeout > 0) {
            terminateTimeoutID = setTimeout(function() {
                // Clear our ID since we're now running
                terminateTimeoutID = undefined;
